...
export function ParentBadge(props: { issue: IssueSummary; compact?: boolean }) {
  ...
  const color = () => {
    ...
    return state.issueTypes.find((candidate) => candidate.id === type || candidate.name === type)?.color ?? issueTypeColorForName(type)
  }
  const label = () => {
    const value = parent()
