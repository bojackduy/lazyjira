...
import { issueTypeColor, statusById } from "../state/selectors"
import {
  cycleTimelineZoom,
  formatTimelineDate,
  panTimelineWindow,
  ...
  return (
    <box flexDirection="column" gap={1} flexGrow={1} minHeight={0} overflow="hidden">
      <box flexDirection="column" flexShrink={0}>
        ...
        <text fg={theme.textMuted} wrapMode="none">j/k row · g/G ends · Ctrl-u/d half page · h/l pan · [/] viewport · Space collapse · z zoom · t today · Enter detail · L load more</text>
        ...
        <For each={timelineNotices(model())}>{(notice) => <text fg={theme.warning} wrapMode="none">{notice}</text>}</For>
      </box>

      <Show when={rows().length} fallback={<TimelineEmptyState text={timelineStateText(state, model())} />}>
...
function WideTimelineRow(props: { row: TimelineProjectedRow; previousGroup?: TimelineProjectedRow["group"]; cells: TimelineCell[]; identityWidth: number; cellWidth: number }) {
  ...
  const schedule = () => timelineSchedule(props.row.issue, props.cells)
      ...
      <For each={props.rows}>{(row, index) => (
        <>
          ...
          <box id={`timeline-${row.issue.key}`} flexDirection="column" flexShrink={0} paddingLeft={1 + row.depth * 2} backgroundColor={state.timelineSelectedIssueKey === row.issue.key && state.focusedPane === "main" ? theme.selected : undefined}>
            ...
            <text fg={row.issue.blocked ? theme.warning : row.group === "invalid-hierarchy" ? theme.danger : theme.textMuted} wrapMode="none">  {timelineRowCopy(row)}</text>
          </box>
        </>
      )}</For>
    </scrollbox>
...
function TimelineEmptyState(props: { text: string }) {
  const theme = useTheme()
  return <box flexGrow={1} alignItems="center" justifyContent="center" flexDirection="column" gap={1}><text fg={theme.textMuted}>{props.text}</text><text fg={theme.textSubtle}>P switch project · q quit</text></box>
}
