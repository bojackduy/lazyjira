...
export function paletteCommandsForBoard(board: BoardType | { type: BoardType } = "scrum"): readonly PaletteCommand[] {
  ...
  return [
    ...
    command("issue.assign", "Assign issue", "Choose an assignable Jira user for the selected issue.", "a", "Jira"),
    command("issue.status", "Change status", "Choose a valid status for the selected issue.", "s", "Jira"),
