...
export function normalizeJiraIssues(issues: JiraIssue[], statuses: StatusDefinition[], options: { fallbackSprintId?: string; fieldIds?: JiraIssueFieldIds; priorityColors?: ReadonlyMap<string, string> } = {}): IssueSummary[] {
  ...
  return issues.flatMap((issue) => {
    ...
    return [{
      ...
      type: fields.issuetype?.id ?? fields.issuetype?.name ?? "Task",
      ...
      parent: fields.parent?.key ? {
        ...
        type: fields.parent.fields?.issuetype?.id ?? fields.parent.fields?.issuetype?.name,
        typeName: fields.parent.fields?.issuetype?.name,
        typeHierarchyLevel: fields.parent.fields?.issuetype?.hierarchyLevel,
      } : undefined,
